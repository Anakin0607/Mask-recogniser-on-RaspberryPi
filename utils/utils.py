...
import sys
#-----------------------------#
#   calc the zoom proportion of the original imgs
# 
#-----------------------------#
...
def detect_face_12net(cls_prob,roi,out_side,scale,width,height,threshold):
    ...
    stride = 0
    # stride around equal 2
    if out_side != 1:
        stride = float(2*out_side-1)/(out_side-1)
    (x,y) = np.where(cls_prob>=threshold)

    boundingbox = np.array([x,y]).T
    # find the location in the original img
    ...
    return NMS(pick,0.3)
#-----------------------------#
#   change the rectangle to square
#-----------------------------#
...
def Alignment_1(img,landmark):
    ...
    # The inclination angle of the eye line relative to the horizontal
    if x==0:
        angle = 0
    else: 
        # Calculate the radian system
